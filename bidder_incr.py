...
class Bidder:
    ...
    def bid(self, user_id):
        ...
        if self.user_id not in self.user_click_history and self.user_id not in self.price_history:
            self.bid_list[self.user_id] =  0

        elif self.user_id not in self.user_click_history and self.user_id in self.price_history:
            self.bid_list[self.user_id] = min(self.bid_list[self.user_id] + alpha*np.median(self.price_history[self.user_id]),np.median(self.price_history[self.user_id])) 
