...
class Bidder:
    ...
    def __init__(self, num_users, num_rounds):
        ...
        Bidder.bid_num +=1
    def __repr__(self):
        return "Hybrid EG & PD model #" + str(self.bid_num)

    def __str__(self):
        return "Hybrid EG & PD model"
    ...
    def bid(self, user_id):
        ...
        if self.current_round < self.num_rounds/2:
            ...
            #epsilon greedy takes over after halfway mark in game.
        else:
            self.user_id = user_id
            ...
            if self.user_id not in self.user_click_history:
                bid_amount = np.random.choice([0,1],p=[greedy,epsilon])
            
            else:
                bid_amount = np.random.choice([np.average(self.user_click_history[self.user_id]),1], p = [greedy, epsilon])
