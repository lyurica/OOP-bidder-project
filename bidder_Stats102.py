...
class Bidder:
    ...
    def bid(self, user_id):
        ...
        #posterior likelihood of bernoulli distribution with uniform[0,1] prior
        
        self.user_id = user_id


        if self.user_id not in self.user_click_history and self.user_id in self.price_history:
            bid_amount = min(0.5, np.average(self.price_history[self.user_id]))
            
        elif self.user_id not in self.user_click_history and self.user_id not in self.price_history:
            bid_amount = 0.5
