...
class Bidder:
    ...
    def __init__(self, num_users, num_rounds):
        ...
        Bidder.bid_num +=1
    def __repr__(self):
        return "Epsilon Greedy Bidder #" + str(self.bid_num)

    def __str__(self):
        return "Epsilon Greedy Bidder"
    ...
    def bid(self, user_id):
        ...
        epsilon = 0.1 -0.1*(self.current_round/self.num_rounds)
        greedy = 1-epsilon

        if self.user_id not in self.user_click_history and self.user_id in self.price_history:
            bid_amount = np.random.choice([0,self.price_history[self.user_id][-1]],p=[greedy,epsilon])
        
        elif self.user_id not in self.user_click_history:
            bid_amount = np.random.choice([0,0.5],p=[greedy,epsilon])
        
        else:
            bid_amount = np.random.choice([0.9*np.average(self.user_click_history[self.user_id]),np.median(self.price_history[self.user_id])], p = [greedy, epsilon])
